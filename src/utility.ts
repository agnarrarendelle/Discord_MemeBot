...
const setUpCommandReact = async (message: discord.Message) => {
  ...
  //get the user input message and remove the spaces in the beginning and the end
  //as well as remove the prefix !
  const userInput = message.content.replace(prefix, "").trim();
  //call getCommands with the parsed input to create a closure
  //that can be used to retrieve splited commands with index
  const userCommands = getCommands(userInput);
  //get the first string, which is the type of meme images to fetch
  const command = userCommands(0);
  ...
  if (sendTimes > sendTimeRange.max) {
    message.channel.send(
      "Maximum send times exceeded. Please enter a smaller value"
    );
    return;
  }
...
//create a closure that lives an array generated by splitting userInput with white spaces
//and returns a function that can be used to retrieve element of the array with index
